...
static int run_mul_full_workspace_depth_scout_batch_step(
  ...
  if (lane == 0) {
    ...
    for (unsigned int index = 0; ok && index < batch; ++index) {
      for (size_t family = 0; ok && family < XRAY_MUL_OPERAND_FAMILIES; ++family) {
        ok = (candidate_interp_flags & XRAY_BENCH_TOOM_INTERP_DIV3) ?
          xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div3_probe(
            &candidate_out[family],
            &left[family],
            &right[family],
            candidate_leaf_threshold,
            candidate_depth_limit) :
          ((candidate_interp_flags & XRAY_BENCH_TOOM_INTERP_DIV2) ?
            xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_probe(
              &candidate_out[family],
              &left[family],
              &right[family],
              candidate_leaf_threshold,
              candidate_depth_limit) :
            xray_bigint_mul_toom3_unroll4_recursive_full_workspace_probe(
              &candidate_out[family],
              &left[family],
              &right[family],
              candidate_leaf_threshold,
