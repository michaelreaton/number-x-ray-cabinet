...
static void append_bigint_route_config_json(JsonBuffer *buffer) {
  ...
  jb_printf(buffer,
    "\"scratchRouteConfig\":{\"wordBits\":%u,\"karatsubaThresholdLimbs\":%zu,\"squareTinySelfMulPolicy\":\"<=8 limbs\",\"decimalHornerMinLimbs\":%zu,\"decimalPairWriterPolicy\":\"small<=8 limbs or horner 48..54 limbs\",\"decimalDcPolicy\":\"base-1e19 D&C at >=4096 digits, leaf=8 chunks\",\"mulUnroll4RouteMinLimbs\":%zu,\"mulUnroll4RouteMaxLimbs\":%zu,\"mulUnroll4RouteEnabled\":%s,\"msvcUint128Helpers\":%s}",
    route.word_bits,
...
static void benchmark_frontier_label(const XrayBenchmarkResult *row, char *out, size_t out_size) {
  ...
  if (benchmark_detail_value(row, "chunkDigits", value, sizeof(value))) {
    append_label_token(out, out_size, "chunk=", value);
  }
...
char *xray_benchmark_frontier_text(const XrayBenchmarkReport *report) {
  ...
  XrayBigIntRouteConfig route = xray_bigint_route_config();
  jb_printf(&buffer,
    "Bigint route: word=%ub | karatsuba>=%zu limbs | square-self-mul<=8 limbs | format-horner>=%zu limbs | format-pair-writer=small<=8 or horner 48..54 limbs | format-dc>=4096 digits leaf=8 | mul-unroll4=%s %zu..%zu limbs | msvc128=%s\n",
    route.word_bits,
    route.karatsuba_threshold_limbs,
    route.decimal_horner_min_limbs,
