...
static void test_benchmark_progress_digest(void) {
  ...
  CHECK(strstr(digest, "baseline/current, duplicate-control, noisy-control, and product-gated rows") != NULL);
  xray_free(digest);
  free(tsv);
}
...
static void test_benchmarks(void) {
  ...
  for (size_t index = 0; index < report->result_count; ++index) {
    ...
    if (strcmp(report->results[index].category, "scratch-vs-gmp") == 0) {
      ...
      CHECK(strstr(report->results[index].detail, "mfastKnob=ntt16_wide61_difdit_32000") != NULL);
