...
static void test_scratch_bigint_oracle(void) {
  ...
  CHECK(xray_bigint_add(&a, &a, &b));
  check_scratch_matches_mpz(&a, gsum);
  CHECK(xray_bigint_set_decimal(&a, large_tail_left));
  CHECK(xray_bigint_set_decimal(&b, large_tail_right));
  CHECK(xray_bigint_add(&b, &a, &b));
  check_scratch_matches_mpz(&b, gsum);
  CHECK(xray_bigint_set_decimal(&a, large_tail_left));
  CHECK(xray_bigint_set_decimal(&b, large_tail_right));
  CHECK(xray_bigint_sub(&difference, &a, &b));
...
static void test_scratch_bigint_dense_leaf_probe_oracle(void) {
  const size_t thresholds[] = {32, 64, 96, 128};
  char *left_text = make_pattern_decimal(2600, "93751086420975318642");
  char *right_text = make_pattern_decimal(2600, "46297531864208642135");
  ...
  for (size_t index = 0; index < sizeof(thresholds) / sizeof(thresholds[0]); ++index) {
    CHECK(xray_bigint_mul_dense_leaf_probe(&product, &a, &b, thresholds[index]));
    check_scratch_matches_mpz(&product, gproduct);
    CHECK(xray_bigint_mul_with_threshold(&baseline, &a, &b, thresholds[index]));
    CHECK(xray_bigint_compare(&product, &baseline) == 0);

    CHECK(xray_bigint_copy(&alias, &a));
    CHECK(xray_bigint_mul_dense_leaf_probe(&alias, &alias, &b, thresholds[index]));
    check_scratch_matches_mpz(&alias, gproduct);
  }
...
static void test_scratch_bigint_karatsuba_workspace_probe_oracle(void) {
  const size_t thresholds[] = {32, 64, 96, 128};
  char *left_text = make_pattern_decimal(3600, "97531086429753108642");
  char *right_text = make_pattern_decimal(3600, "42086429753108642975");
  XrayScratchBigInt a, b, product, baseline, alias_left, alias_right;
  xray_bigint_init(&a);
  xray_bigint_init(&b);
  xray_bigint_init(&product);
  xray_bigint_init(&baseline);
  ...
  for (size_t index = 0; index < sizeof(thresholds) / sizeof(thresholds[0]); ++index) {
    CHECK(xray_bigint_mul_karatsuba_workspace_probe(&product, &a, &b, thresholds[index]));
    check_scratch_matches_mpz(&product, gproduct);
    CHECK(xray_bigint_mul_with_threshold(&baseline, &a, &b, thresholds[index]));
    CHECK(xray_bigint_compare(&product, &baseline) == 0);

    CHECK(xray_bigint_copy(&alias_left, &a));
    CHECK(xray_bigint_mul_karatsuba_workspace_probe(&alias_left, &alias_left, &b, thresholds[index]));
    check_scratch_matches_mpz(&alias_left, gproduct);

    CHECK(xray_bigint_copy(&alias_right, &b));
    CHECK(xray_bigint_mul_karatsuba_workspace_probe(&alias_right, &a, &alias_right, thresholds[index]));
...
static void test_scratch_bigint_toom3_recursive_probe_oracle(void) {
  ...
  xray_bigint_init(&full_workspace_product);
  xray_bigint_init(&full_workspace_div2_product);
  xray_bigint_init(&full_workspace_div3_product);
  ...
  xray_bigint_init(&full_workspace_reuse_neg2_product);
  xray_bigint_init(&full_workspace_reuse_inplace_product);
  xray_bigint_init(&full_workspace_toom4_top_product);
  ...
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_view_probe(&view_product, &a, &b, 64, 2));
  ...
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_probe(&full_workspace_product, &a, &b, 64, 2));
  check_scratch_matches_mpz(&full_workspace_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_product, &view_product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_product, &workspace_product) == 0);

  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_probe(&full_workspace_div2_product, &a, &b, 64, 2));
  check_scratch_matches_mpz(&full_workspace_div2_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_div2_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_div2_product, &full_workspace_product) == 0);

  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div3_probe(&full_workspace_div3_product, &a, &b, 64, 2));
  check_scratch_matches_mpz(&full_workspace_div3_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_div3_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_div3_product, &full_workspace_product) == 0);

  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_div3_probe(&full_workspace_combo_product, &a, &b, 64, 2));
  check_scratch_matches_mpz(&full_workspace_combo_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_combo_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_combo_product, &full_workspace_product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_combo_product, &full_workspace_div2_product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_combo_product, &full_workspace_div3_product) == 0);

  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_inplace_div2_div3_probe(&full_workspace_inplace_combo_product, &a, &b, 64, 2));
  check_scratch_matches_mpz(&full_workspace_inplace_combo_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_inplace_combo_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_inplace_combo_product, &full_workspace_product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_inplace_combo_product, &full_workspace_combo_product) == 0);

  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_reuse_div2_div3_probe(&full_workspace_reuse_product, &a, &b, 64, 2, &mul_workspace));
  ...
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_reuse_neg2_div2_div3_probe(&full_workspace_reuse_neg2_product, &a, &b, 64, 2, &mul_workspace));
  check_scratch_matches_mpz(&full_workspace_reuse_neg2_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_reuse_neg2_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_reuse_neg2_product, &full_workspace_combo_product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_reuse_neg2_product, &full_workspace_reuse_product) == 0);
  ...
  CHECK(xray_bigint_mul_toom4_top_full_workspace_probe(&full_workspace_toom4_top_product, &a, &b, 48, 3));
  check_scratch_matches_mpz(&full_workspace_toom4_top_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_toom4_top_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_toom4_top_product, &full_workspace_combo_product) == 0);

  CHECK(xray_bigint_mul_toom4_top_full_workspace_reuse_probe(&full_workspace_toom4_top_reuse_product, &a, &b, 48, 3, &mul_workspace));
  check_scratch_matches_mpz(&full_workspace_toom4_top_reuse_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_toom4_top_reuse_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_toom4_top_reuse_product, &full_workspace_combo_product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_toom4_top_reuse_product, &full_workspace_toom4_top_product) == 0);

  CHECK(xray_bigint_mul_toom4_top_full_workspace_reuse_probe(&full_workspace_toom4_top_reuse_l64d2_product, &a, &b, 64, 2, &mul_workspace));
  check_scratch_matches_mpz(&full_workspace_toom4_top_reuse_l64d2_product, gproduct);
  CHECK(xray_bigint_compare(&full_workspace_toom4_top_reuse_l64d2_product, &product) == 0);
  CHECK(xray_bigint_compare(&full_workspace_toom4_top_reuse_l64d2_product, &full_workspace_toom4_top_reuse_product) == 0);

  CHECK(xray_bigint_mul_toom4_top_full_workspace_reuse_factored_div_probe(&full_workspace_toom4_top_factored_div_product, &a, &b, 48, 3, &mul_workspace));
  ...
  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_view_probe(&alias, &alias, &b, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);
  ...
  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_probe(&alias, &alias, &b, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_probe(&alias, &a, &alias, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_probe(&alias, &alias, &b, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_probe(&alias, &a, &alias, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div3_probe(&alias, &alias, &b, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div3_probe(&alias, &a, &alias, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_div3_probe(&alias, &alias, &b, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_div3_probe(&alias, &a, &alias, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_inplace_div2_div3_probe(&alias, &alias, &b, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_inplace_div2_div3_probe(&alias, &a, &alias, 64, 2));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_reuse_div2_div3_probe(&alias, &alias, &b, 64, 2, &mul_workspace));
  check_scratch_matches_mpz(&alias, gproduct);
  ...
  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_reuse_neg2_div2_div3_probe(&alias, &alias, &b, 64, 2, &mul_workspace));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom3_unroll4_recursive_full_workspace_reuse_neg2_div2_div3_probe(&alias, &a, &alias, 64, 2, &mul_workspace));
  check_scratch_matches_mpz(&alias, gproduct);
  ...
  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom4_top_full_workspace_probe(&alias, &alias, &b, 48, 3));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom4_top_full_workspace_probe(&alias, &a, &alias, 48, 3));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom4_top_full_workspace_reuse_probe(&alias, &alias, &b, 48, 3, &mul_workspace));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &b));
  CHECK(xray_bigint_mul_toom4_top_full_workspace_reuse_probe(&alias, &a, &alias, 48, 3, &mul_workspace));
  check_scratch_matches_mpz(&alias, gproduct);

  CHECK(xray_bigint_copy(&alias, &a));
  CHECK(xray_bigint_mul_toom4_top_full_workspace_reuse_factored_div_probe(&alias, &alias, &b, 48, 3, &mul_workspace));
  check_scratch_matches_mpz(&alias, gproduct);
  ...
  xray_bigint_clear(&full_workspace_product);
  xray_bigint_clear(&full_workspace_div2_product);
  xray_bigint_clear(&full_workspace_div3_product);
  ...
  xray_bigint_clear(&full_workspace_reuse_neg2_product);
  xray_bigint_clear(&full_workspace_reuse_inplace_product);
  xray_bigint_clear(&full_workspace_toom4_top_product);
  ...
  CHECK(!xray_bigint_mul_toom3_unroll4_recursive_full_workspace_probe(&value, &value, &value, 64, 2));
  CHECK(!xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_probe(&value, &value, &value, 64, 2));
  CHECK(!xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div3_probe(&value, &value, &value, 64, 2));
  ...
  CHECK(!xray_bigint_mul_toom3_unroll4_recursive_full_workspace_reuse_neg2_div2_div3_probe(&value, &value, &value, 64, 2, &mul_workspace));
...
static void test_scratch_bigint_toom5_top_reuse_probe_oracle(void) {
#if defined(_MSC_VER) && defined(_M_X64)
  char *left_text = make_pattern_decimal(1000, "98765432101234567890");
  char *right_text = make_pattern_decimal(1000, "27182818284590452353");
  XrayScratchBigInt a, b, current_product, toom5_product, alias;
  XrayBigIntMulWorkspace mul_workspace;
  xray_bigint_init(&a);
  xray_bigint_init(&b);
  xray_bigint_init(&current_product);
  xray_bigint_init(&toom5_product);
  xray_bigint_init(&alias);
  xray_bigint_mul_workspace_init(&mul_workspace);
  mpz_t ga, gb, gproduct;
  mpz_inits(ga, gb, gproduct, NULL);
  ...
  CHECK(xray_bigint_mul(&current_product, &a, &b));
  check_scratch_matches_mpz(&current_product, gproduct);

  CHECK(xray_bigint_mul_toom5_top_full_workspace_reuse_probe(&toom5_product, &a, &b, 8, 1, &mul_workspace));
  ...
  xray_bigint_clear(&a);
  xray_bigint_clear(&b);
  xray_bigint_clear(&current_product);
  xray_bigint_clear(&toom5_product);
  xray_bigint_clear(&alias);
  mpz_clears(ga, gb, gproduct, NULL);
  free(left_text);
  free(right_text);
...
static void test_benchmarks(void) {
  ...
  CHECK(events.saw_benchmark_row_large_mul_campaign);
#if defined(_MSC_VER) && defined(_M_X64)
  ...
  int saw_mul_dense_leaf_vs_scan_probe = 0;
  ...
  int saw_mul_large_cpu_campaign_probe = 0;
  int saw_mul_large_cpu_campaign4096_probe = 0;
  int saw_mul_large_cpu_campaign5639_probe = 0;
  int saw_mul_large_cpu_campaign8192_probe = 0;
  int saw_mul_large_cpu_campaign11717_probe = 0;
  int saw_mul_large_cpu_campaign16384_probe = 0;
  int saw_mul_large_cpu_campaign24103_probe = 0;
  int saw_mul_large_cpu_campaign32768_probe = 0;
  int saw_mul_large_cpu_campaign52163_probe = 0;
  int saw_mul_large_cpu_campaign65536_probe = 0;
#if defined(_MSC_VER) && defined(_M_X64)
  int saw_mul_large_cpu_toom_branch_probe = 0;
  int saw_mul_large_cpu_toom_branch4096_probe = 0;
  int saw_mul_large_cpu_toom_branch5639_probe = 0;
  int saw_mul_large_cpu_toom_branch8192_probe = 0;
  int saw_mul_large_cpu_toom_branch11717_probe = 0;
  int saw_mul_large_cpu_toom_branch16384_probe = 0;
  int saw_mul_large_cpu_toom_branch24103_probe = 0;
  int saw_mul_large_cpu_toom_branch32768_probe = 0;
  int saw_mul_large_cpu_toom_branch52163_probe = 0;
  int saw_mul_large_cpu_toom_branch65536_probe = 0;
  int saw_mul_large_cpu_toom_view_branch_probe = 0;
  int saw_mul_large_cpu_toom_view_branch4096_probe = 0;
  int saw_mul_large_cpu_toom_view_branch5639_probe = 0;
  int saw_mul_large_cpu_toom_view_branch8192_probe = 0;
  int saw_mul_large_cpu_toom_view_branch11717_probe = 0;
  int saw_mul_large_cpu_toom_view_branch16384_probe = 0;
  int saw_mul_large_cpu_toom_view_branch24103_probe = 0;
  ...
  int saw_mul_large_cpu_toom_full_workspace_probe = 0;
  int saw_mul_large_cpu_toom_full_workspace4096_probe = 0;
  int saw_mul_large_cpu_toom_full_workspace5639_probe = 0;
  int saw_mul_large_cpu_toom_full_workspace8192_probe = 0;
  int saw_mul_large_cpu_toom_full_workspace11717_probe = 0;
  int saw_mul_large_cpu_toom_full_workspace16384_probe = 0;
  int saw_mul_large_cpu_toom_full_workspace24103_probe = 0;
  ...
  int saw_mul_large_cpu_toom_full_deep_point_probe = 0;
  int saw_mul_large_cpu_toom_full_deep_point11717_probe = 0;
  int saw_mul_large_cpu_toom_full_deep_point16384_probe = 0;
  int saw_mul_large_cpu_toom_full_deep_point24103_probe = 0;
  int saw_mul_large_cpu_toom_full_deep_point32768_probe = 0;
  int saw_mul_large_cpu_toom_full_deep_point52163_probe = 0;
  int saw_mul_large_cpu_toom_full_deep_point65536_probe = 0;
  int saw_mul_large_cpu_toom_full_deep_audit_probe = 0;
  int saw_mul_large_cpu_toom_depth_point_probe = 0;
  int saw_mul_large_cpu_toom_depth_point11717_probe = 0;
  int saw_mul_large_cpu_toom_depth_point16384_probe = 0;
  int saw_mul_large_cpu_toom_depth_point24103_probe = 0;
  int saw_mul_large_cpu_toom_depth_point32768_probe = 0;
  int saw_mul_large_cpu_toom_depth_point52163_probe = 0;
  int saw_mul_large_cpu_toom_depth_point65536_probe = 0;
  int saw_mul_large_cpu_toom_depth_scout_probe = 0;
  int saw_mul_large_cpu_toom_leaf_point_probe = 0;
  int saw_mul_large_cpu_toom_leaf_point11717_probe = 0;
  int saw_mul_large_cpu_toom_leaf_point16384_probe = 0;
  int saw_mul_large_cpu_toom_leaf_point24103_probe = 0;
  int saw_mul_large_cpu_toom_leaf_point32768_probe = 0;
  int saw_mul_large_cpu_toom_leaf_point52163_probe = 0;
  int saw_mul_large_cpu_toom_leaf_point65536_probe = 0;
  int saw_mul_large_cpu_toom_leaf_scout_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_point_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_point11717_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_point16384_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_point24103_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_point32768_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_point52163_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_point65536_probe = 0;
  int saw_mul_large_cpu_toom_leaf48_scout_probe = 0;
  int saw_mul_large_cpu_toom_div2_point_probe = 0;
  int saw_mul_large_cpu_toom_div2_point11717_probe = 0;
  int saw_mul_large_cpu_toom_div2_point16384_probe = 0;
  int saw_mul_large_cpu_toom_div2_point24103_probe = 0;
  int saw_mul_large_cpu_toom_div2_point32768_probe = 0;
  int saw_mul_large_cpu_toom_div2_point52163_probe = 0;
  int saw_mul_large_cpu_toom_div2_point65536_probe = 0;
  int saw_mul_large_cpu_toom_div2_scout_probe = 0;
  int saw_mul_large_cpu_toom_div3_point_probe = 0;
  int saw_mul_large_cpu_toom_div3_point11717_probe = 0;
  int saw_mul_large_cpu_toom_div3_point16384_probe = 0;
  int saw_mul_large_cpu_toom_div3_point24103_probe = 0;
  int saw_mul_large_cpu_toom_div3_point32768_probe = 0;
  int saw_mul_large_cpu_toom_div3_point52163_probe = 0;
  int saw_mul_large_cpu_toom_div3_point65536_probe = 0;
  int saw_mul_large_cpu_toom_div3_scout_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_point_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_point11717_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_point16384_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_point24103_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_point32768_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_point52163_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_point65536_probe = 0;
  int saw_mul_large_cpu_toom_div2_div3_scout_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_leaf48_scout_probe = 0;
  int saw_mul_large_cpu_toom_cmb_depth3_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_depth3_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_depth3_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_depth3_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_depth3_point32768_probe = 0;
  ...
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point4096_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point5639_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point8192_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d3_full_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point4096_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point5639_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point8192_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_hand_point32768_probe = 0;
  ...
  int saw_mul_large_cpu_toom_cmb_tournament_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_tournament_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_tournament_l64d2_probe = 0;
  int saw_mul_large_cpu_toom_cmb_tournament_l48d3_probe = 0;
  int saw_mul_large_cpu_toom_cmb_tournament_l48d4_probe = 0;
  ...
  int saw_mul_large_cpu_toom_cmb_reuse_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point4096_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point5639_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point8192_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_reuse_map_point32768_probe = 0;
  ...
  int saw_mul_large_cpu_toom_cmb_transition_route_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_transition_route_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_transition_route_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_transition_route_probe = 0;
  int saw_mul_large_cpu_toom_cmb_transition_control_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_transition_control_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_transition_control_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_transition_control_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_control_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_control_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_control_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_control_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_control_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_control_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_transition_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_transition_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_transition_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_gmp_transition_probe = 0;
  int saw_mul_large_cpu_toom_cmb_neg2_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_neg2_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_neg2_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_neg2_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_neg2_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_neg2_probe = 0;
  int saw_mul_large_cpu_toom5_top_reuse_point_probe = 0;
  ...
  int saw_mul_large_cpu_toom_cmb_map_point8192_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_control_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_control_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_control_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_map_control_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d4_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d4_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d4_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d4_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d4_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l48d4_scout_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l32d4_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l32d4_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l32d4_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l32d4_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l32d4_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l32d4_scout_probe = 0;
  int saw_mul_large_cpu_toom_cmb_ipdiv_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_ipdiv_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_ipdiv_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_ipdiv_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_ipdiv_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_ipdiv_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l64d3_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l64d3_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l64d3_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l64d3_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l64d3_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_l64d3_scout_probe = 0;
  int saw_mul_large_cpu_toom4_top_point_probe = 0;
  int saw_mul_large_cpu_toom4_top_point24103_probe = 0;
  int saw_mul_large_cpu_toom4_top_point32768_probe = 0;
  int saw_mul_large_cpu_toom4_top_point52163_probe = 0;
  int saw_mul_large_cpu_toom4_top_point65536_probe = 0;
  int saw_mul_large_cpu_toom4_top_scout_probe = 0;
  int saw_mul_large_cpu_toom4_top_reuse_point_probe = 0;
  int saw_mul_large_cpu_toom4_top_reuse_point24103_probe = 0;
  int saw_mul_large_cpu_toom4_top_reuse_point32768_probe = 0;
  int saw_mul_large_cpu_toom4_top_reuse_point52163_probe = 0;
  int saw_mul_large_cpu_toom4_top_reuse_point65536_probe = 0;
  int saw_mul_large_cpu_toom4_top_reuse_scout_probe = 0;
  int saw_mul_large_cpu_toom4_top_handoff_point_probe = 0;
  int saw_mul_large_cpu_toom4_top_handoff_point24103_probe = 0;
  int saw_mul_large_cpu_toom4_top_handoff_point32768_probe = 0;
  int saw_mul_large_cpu_toom4_top_handoff_point52163_probe = 0;
  int saw_mul_large_cpu_toom4_top_handoff_point65536_probe = 0;
  int saw_mul_large_cpu_toom4_top_handoff_scout_probe = 0;
  int saw_mul_large_cpu_toom4_top_fdiv_point_probe = 0;
  int saw_mul_large_cpu_toom4_top_fdiv_point24103_probe = 0;
  int saw_mul_large_cpu_toom4_top_fdiv_point32768_probe = 0;
  int saw_mul_large_cpu_toom4_top_fdiv_point52163_probe = 0;
  int saw_mul_large_cpu_toom4_top_fdiv_point65536_probe = 0;
  int saw_mul_large_cpu_toom4_top_fdiv_scout_probe = 0;
  int saw_mul_large_cpu_toom4_top_vs_cmb_point_probe = 0;
  int saw_mul_large_cpu_toom4_top_vs_cmb_point24103_probe = 0;
  int saw_mul_large_cpu_toom4_top_vs_cmb_point32768_probe = 0;
  int saw_mul_large_cpu_toom4_top_vs_cmb_point52163_probe = 0;
  int saw_mul_large_cpu_toom4_top_vs_cmb_point65536_probe = 0;
  int saw_mul_large_cpu_toom4_top_vs_cmb_scout_probe = 0;
  ...
  int saw_mul_large_cpu_toom_cmb_lower_point8192_probe = 0;
  int saw_mul_large_cpu_toom_cmb_lower_scout_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point4096_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point5639_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point8192_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point11717_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point16384_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point24103_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point32768_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point52163_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_point65536_probe = 0;
  int saw_mul_large_cpu_toom_cmb_route_audit_probe = 0;
#endif
  ...
  for (size_t index = 0; index < report->result_count; ++index) {
    ...
    if (strcmp(report->results[index].category, "scratch-vs-gmp") == 0) {
      ...
      if (strcmp(report->results[index].operation, "mul-unroll4-deep-vs-gmp") == 0 ||
          ...
          strcmp(report->results[index].operation, "mul-large-toom-full-deep-point") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-depth-point") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-leaf-point") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-leaf48-point") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-div2-point") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-div3-point") == 0 ||
          ...
          strcmp(report->results[index].operation, "mul-large-toom-cmb-gmptrans-pt") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-cmb-neg2-pt") == 0 ||
          ...
          strcmp(report->results[index].operation, "mul-large-toom-cmb-l32d4-point") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-cmb-ipdiv-pt") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom-cmb-l64d3-point") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom4-top-pt") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom4-top-reuse-pt") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom4-top-handoff-pt") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom4-top-fdiv-pt") == 0 ||
          strcmp(report->results[index].operation, "mul-large-toom4-top-vs-cmb-pt") == 0 ||
      ...
      if (strcmp(report->results[index].operation, "mul-dense-leaf-vs-scan") == 0) {
        saw_mul_dense_leaf_vs_scan_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_dense_leaf_vs_scan4096_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_dense_leaf_vs_scan8192_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_dense_leaf_vs_scan16384_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=dense-leaf-no-sparse-scan") != NULL);
        CHECK(strstr(report->results[index].detail, "threshold-mul-with-sparse-scan") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-cpu-campaign") == 0) {
        saw_mul_large_cpu_campaign_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_campaign4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_campaign5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_campaign8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_campaign11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_campaign16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_campaign24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_campaign32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_campaign52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_campaign65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=karatsuba-split-view-workspace") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "comparison=current-default+split-view+workspace+mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-campaign") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=mpn-slice-view-recursion+scratch-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "sourceLabel=workspace-split-view") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
...
#if defined(_MSC_VER) && defined(_M_X64)
      if (strcmp(report->results[index].operation, "mul-large-cpu-toom-branch") == 0) {
        saw_mul_large_cpu_toom_branch_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_branch4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_branch5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_branch8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_branch11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_branch16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_branch24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_branch32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_branch52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_branch65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        ...
        CHECK(strstr(report->results[index].detail, "candidate=recursive-toom3+unroll4") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "comparison=current-default+workspace+recursive-toom3+mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-branch") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-recursive") != NULL);
        CHECK(strstr(report->results[index].detail, "toomWorkspaceRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "toomGmpRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        if (report->results[index].digits == 4096 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-cpu-toom-view-branch") == 0) {
        saw_mul_large_cpu_toom_view_branch_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_view_branch4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_view_branch5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_view_branch8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_view_branch11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_view_branch16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_view_branch24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_view_branch32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_view_branch52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_view_branch65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=recursive-toom3+unroll4-split-view") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=recursive-toom3+unroll4-copy-slices") != NULL);
        CHECK(strstr(report->results[index].detail, "comparison=toom-copy+current+workspace+mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-view-branch") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-split-view-copy-tax") != NULL);
        CHECK(strstr(report->results[index].detail, "ratioMethod=paired-median") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-cpu-toom-full-ws") == 0) {
        saw_mul_large_cpu_toom_full_workspace_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_full_workspace4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_full_workspace5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_full_workspace8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_full_workspace11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_full_workspace16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_full_workspace24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_full_workspace32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_full_workspace52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_full_workspace65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        ...
        CHECK(strstr(report->results[index].detail, "baseline=recursive-toom3+unroll4-workspace") != NULL);
        CHECK(strstr(report->results[index].detail, "comparison=full-ws+toom-ws+view+copy+current+karatsuba-ws+mpz") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-full-ws") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-recursive-karatsuba-temp-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "ratioMethod=paired-median") != NULL);
        CHECK(strstr(report->results[index].detail, "fullWsCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "fullWsToomWsRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "fullWsViewRatio=") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-full-deep-point") == 0) {
        saw_mul_large_cpu_toom_full_deep_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_full_deep_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_full_deep_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_full_deep_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_full_deep_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_full_deep_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_full_deep_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-full-deep-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=full-deep-audit") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "currentWorst=") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpWorst=") != NULL);
        CHECK(strstr(report->results[index].detail, "ratioMethod=paired-median") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-depth-point") == 0) {
        saw_mul_large_cpu_toom_depth_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_depth_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_depth_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_depth_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_depth_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_depth_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_depth_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-depth-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=depth-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-depth3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-depth2") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-depth-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-depth-limit") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 16384 ||
            report->results[index].digits == 32768 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-leaf-point") == 0) {
        saw_mul_large_cpu_toom_leaf_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_leaf_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_leaf_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_leaf_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_leaf_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_leaf_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_leaf_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-leaf-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=leaf-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-leaf96-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=64") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=96") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        CHECK(strstr(report->results[index].detail, "requiredStablePairs=8/9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-leaf96") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-leaf64") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-leaf-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-leaf-threshold") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 16384 ||
            report->results[index].digits == 32768 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-leaf48-point") == 0) {
        saw_mul_large_cpu_toom_leaf48_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_leaf48_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_leaf48_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_leaf48_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_leaf48_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_leaf48_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_leaf48_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-leaf48-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=leaf48-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-leaf48-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=64") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-leaf48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-leaf64") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-leaf48-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-leaf-threshold") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 16384 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-div2-point") == 0) {
        saw_mul_large_cpu_toom_div2_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_div2_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_div2_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_div2_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_div2_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_div2_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_div2_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-div2-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=div2-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-div2-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-div2") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-leaf64") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-div2-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-exact-div2") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 16384 ||
            report->results[index].digits == 32768 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-div3-point") == 0) {
        saw_mul_large_cpu_toom_div3_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_div3_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_div3_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_div3_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_div3_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_div3_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_div3_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-div3-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=div3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-div3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-div3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-leaf64") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-div3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-exact-div3") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 16384 ||
            report->results[index].digits == 32768 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-div2-div3-point") == 0) {
        saw_mul_large_cpu_toom_div2_div3_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_div2_div3_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_div2_div3_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_div2_div3_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_div2_div3_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_div2_div3_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_div2_div3_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-div2-div3-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=div2-div3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-div2-div3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-div2-div3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-leaf64") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-div2-div3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-exact-div2-div3") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 16384 ||
            report->results[index].digits == 32768 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-leaf48-point") == 0) {
        saw_mul_large_cpu_toom_cmb_leaf48_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_leaf48_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_leaf48_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_leaf48_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_leaf48_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_leaf48_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_leaf48_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-leaf48-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-leaf48-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-leaf48-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=64") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-leaf48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-leaf64") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-leaf48-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-leaf-threshold") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 16384 ||
            report->results[index].digits == 32768 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-depth3-point") == 0) {
        saw_mul_large_cpu_toom_cmb_depth3_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_depth3_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_depth3_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_depth3_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_depth3_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_depth3_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_depth3_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-depth3-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-depth3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-depth3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=2") != NULL);
        CHECK(strstr(report->results[index].detail, "candDepth=3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=active-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-depth2") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-depth3-scout") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l48d3-fpt") == 0) {
        saw_mul_large_cpu_toom_cmb_l48d3_full_point_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_cmb_l48d3_full_point4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_cmb_l48d3_full_point5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_cmb_l48d3_full_point8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_l48d3_full_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_l48d3_full_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_l48d3_full_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_l48d3_full_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_l48d3_full_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_l48d3_full_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-l48d3-full-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-l48d3-full") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l48d3-full-window-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=64") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-l48d3-full") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-leaf48-depth3-full-window") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 4096 ||
            report->results[index].digits == 8192 ||
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-hand-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_hand_point_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_cmb_hand_point4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_cmb_hand_point5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_cmb_hand_point8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_hand_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_hand_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_hand_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_hand_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_hand_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_hand_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-hand-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-hand-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-handoff-l48d3-ge32768") != NULL);
        CHECK(strstr(report->results[index].detail, "handoffDigits=32768") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-handoff32768") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-tourn-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_tournament_point_probe = 1;
        mul_large_cpu_toom_cmb_tournament_point_rows++;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_tournament_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_tournament_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_tournament_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_tournament_point65536_probe = 1;
        else CHECK(0);
        if (strstr(report->results[index].detail, "route=full-ws-combo-l64d2") != NULL) saw_mul_large_cpu_toom_cmb_tournament_l64d2_probe = 1;
        if (strstr(report->results[index].detail, "route=full-ws-combo-l48d3") != NULL) saw_mul_large_cpu_toom_cmb_tournament_l48d3_probe = 1;
        if (strstr(report->results[index].detail, "route=full-ws-combo-l48d4") != NULL) saw_mul_large_cpu_toom_cmb_tournament_l48d4_probe = 1;
        if (strstr(report->results[index].detail, "route=full-ws-combo-handoff-l64d2-l48d3") != NULL) saw_mul_large_cpu_toom_cmb_tournament_handoff_probe = 1;
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-tourn-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-upper-tournament") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-upper-tournament-ge24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "handoffDigits=32768") != NULL);
        CHECK(strstr(report->results[index].detail, "routes=l64d2,l48d3,l48d4,hand32768") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        CHECK(strstr(report->results[index].detail, "requiredStablePairs=8/9") != NULL);
        CHECK(strstr(report->results[index].detail, "stablePairs=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-upper-tournament") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-reuse-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_reuse_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_reuse_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_reuse_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_reuse_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_reuse_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-reuse-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-reuse-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=tournament-winner-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        CHECK(strstr(report->results[index].detail, "requiredStablePairs=8/9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-reuse-winner") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-nonreuse-winner") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-reuse-workspace") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-reusable-workspace") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits < 52163) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-reuse-map-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_reuse_map_point_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_cmb_reuse_map_point4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_cmb_reuse_map_point5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_cmb_reuse_map_point8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_reuse_map_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_reuse_map_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_reuse_map_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_reuse_map_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_reuse_map_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_reuse_map_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-reuse-map-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-reuse-map-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-map-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=reuse-l64d2-l48d4-l48d3") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=full-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-reuse-map-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-nonreuse-map") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-troute-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_transition_route_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_transition_route_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_transition_route_point16384_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-reuse-map-transition-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-reuse-map-transition-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-map-transition-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=reuse-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "cut24103=leaf48depth4") != NULL);
        CHECK(strstr(report->results[index].detail, "cut52163=leaf48depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-reuse-map-transition") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-reuse-map-transition-window") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-l64d2") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-tctrl-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_transition_control_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_transition_control_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_transition_control_point16384_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].max_allowed_speed_ratio == 1.10);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-reuse-map-transition-control-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-reuse-map-transition-self-control") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "focus=transition-route-noise") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=reuse-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "cut24103=leaf48depth4") != NULL);
        CHECK(strstr(report->results[index].detail, "cut52163=leaf48depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=transition-window") != NULL);
        CHECK(strstr(report->results[index].detail, "duplicateControl=reuse-map-self") != NULL);
        CHECK(strstr(report->results[index].detail, "controlSafety=") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-reuse-map-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-reuse-map-duplicate") != NULL);
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "controlRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "controlWorst=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-reuse-map-transition-control") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-reuse-map-self-control") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-l64d2") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-gmpctrl-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_gmp_control_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_gmp_control_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_gmp_control_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_gmp_control_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_gmp_control_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-gmp-control-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-gmp-control") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=reuse-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "cut24103=leaf48depth4") != NULL);
        CHECK(strstr(report->results[index].detail, "cut52163=leaf48depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "stableGmp=") != NULL);
        CHECK(strstr(report->results[index].detail, "stableCurrent=") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpControlStable=") != NULL);
        CHECK(strstr(report->results[index].detail, "hashSafe=18/18") != NULL);
        CHECK(strstr(report->results[index].detail, "hashGate=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-reuse-map-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=mpz_mul-duplicate") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul-primary") != NULL);
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpControlSafety=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpDuplicateRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "currentGmpRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpControlRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpControlWorst=") != NULL);
        CHECK(strstr(report->results[index].detail, "worstPairRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "ratioMethod=paired-median") != NULL);
        CHECK(strstr(report->results[index].detail, "timingMode=rotating") != NULL);
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-gmp-control") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=mpz-mul-duplicate-control") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits < 52163) {
          CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-l48d4") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-gmptrans-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_gmp_transition_point_probe = 1;
        if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_gmp_transition_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_gmp_transition_point16384_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-gmp-transition-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-gmp-transition-control") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=reuse-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "cut24103=leaf48depth4") != NULL);
        CHECK(strstr(report->results[index].detail, "cut52163=leaf48depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-gmp-control-transition") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=mpz-mul-duplicate-control") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-l64d2") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-neg2-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_neg2_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_neg2_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_neg2_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_neg2_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_neg2_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-reuse-neg2-map-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-reuse-neg2-map-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=reuse-neg2-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "cut24103=leaf48depth4") != NULL);
        CHECK(strstr(report->results[index].detail, "cut52163=leaf48depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-reuse-neg2-l64d2-l48d4-l48d3") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-neg2") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-neg2-eval") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits < 52163) {
          CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-neg2-l48d4") != NULL);
          CHECK(strstr(report->results[index].detail, "depthLimit=4") != NULL);
        } else {
          CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-neg2-l48d3") != NULL);
          CHECK(strstr(report->results[index].detail, "depthLimit=3") != NULL);
        }
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom5-top-reuse-pt") == 0) {
        saw_mul_large_cpu_toom5_top_reuse_point_probe = 1;
        if (report->results[index].digits == 5639) saw_mul_large_cpu_toom5_top_reuse_point5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom5_top_reuse_point8192_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 3);
        CHECK(strstr(report->results[index].detail, "op=mul-toom5-top-reuse-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=toom5-top-vs-combo-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom5-top-reuse-smoke-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=toom5-top-reuse-l32d2-smoke") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=32") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-toom5-top-reuse-l32d2") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-reuse-l32d2") != NULL);
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom5-top-reuse") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-map-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_map_point_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_cmb_map_point4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_cmb_map_point5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_cmb_map_point8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_map_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_map_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_map_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_map_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_map_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_map_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-map-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-best-map-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-best-map-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "cut24103=leaf48depth4") != NULL);
        CHECK(strstr(report->results[index].detail, "cut52163=leaf48depth3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=full-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-best-map-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "lowerCandidate=full-ws-combo-l64d2") != NULL);
        CHECK(strstr(report->results[index].detail, "middleCandidate=full-ws-combo-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "upperCandidate=full-ws-combo-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-best-map") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-full-window-best-map") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits < 24103) {
          CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-l64d2") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-map-ctrl-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_map_control_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_map_control_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_map_control_point32768_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].max_allowed_speed_ratio == 1.10);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-map-control-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-best-map-control") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-best-map-control-24103-32768") != NULL);
        CHECK(strstr(report->results[index].detail, "focus=worst-pair-outliers") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-combo-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=4") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=full-window") != NULL);
        CHECK(strstr(report->results[index].detail, "duplicateControl=best-map-self") != NULL);
        CHECK(strstr(report->results[index].detail, "controlSafety=") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-best-map-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-best-map-duplicate") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "controlRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "controlWorst=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "currentGmpRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "productWorstPairRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "ratioMethod=paired-median") != NULL);
        CHECK(strstr(report->results[index].detail, "timingMode=rotating") != NULL);
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-best-map-control") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-duplicate-control") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768) {
          CHECK(strstr(report->results[index].detail, "sizeRole=power2-anchor") != NULL);
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l48d4-point") == 0) {
        saw_mul_large_cpu_toom_cmb_l48d4_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_l48d4_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_l48d4_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_l48d4_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_l48d4_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-l48d4-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-l48d4-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l48d4-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=3") != NULL);
        CHECK(strstr(report->results[index].detail, "candDepth=4") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        CHECK(strstr(report->results[index].detail, "requiredStablePairs=8/9") != NULL);
        CHECK(strstr(report->results[index].detail, "stableBase=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-l48d4-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-depth4-upper") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l32d4-point") == 0) {
        saw_mul_large_cpu_toom_cmb_l32d4_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_l32d4_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_l32d4_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_l32d4_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_l32d4_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-l32d4-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-l32d4-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l32d4-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=32") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=32") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=4") != NULL);
        CHECK(strstr(report->results[index].detail, "candDepth=4") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        CHECK(strstr(report->results[index].detail, "requiredStablePairs=8/9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-l32d4") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-l32d4-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-leaf32-depth4-upper") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-ipdiv-pt") == 0) {
        saw_mul_large_cpu_toom_cmb_ipdiv_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_ipdiv_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_ipdiv_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_ipdiv_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_ipdiv_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-ipdiv-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-ipdiv-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-inplace-div-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=4") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-inplace-div-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-inplace-interp") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-inplace-exact-division") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l64d3-point") == 0) {
        saw_mul_large_cpu_toom_cmb_l64d3_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_l64d3_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_l64d3_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_l64d3_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_l64d3_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-l64d3-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-l64d3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l64d3-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=64") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=4") != NULL);
        CHECK(strstr(report->results[index].detail, "candDepth=3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-l64d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-l64d3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-leaf64-depth3-upper") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom4-top-pt") == 0) {
        saw_mul_large_cpu_toom4_top_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom4_top_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom4_top_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom4_top_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom4_top_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-toom4-top-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=toom4-top-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=3") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-toom4-top-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom4-top-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom4-top-vs-toom33-upper") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom4-top-reuse-pt") == 0) {
        saw_mul_large_cpu_toom4_top_reuse_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom4_top_reuse_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom4_top_reuse_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom4_top_reuse_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom4_top_reuse_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-toom4-top-reuse-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=toom4-top-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=toom4-top-reuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-toom4-top-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-toom4-top-reuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-toom4-top-nonreuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom4-top-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom4-top-recursive-workspace-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom4-top-handoff-pt") == 0) {
        saw_mul_large_cpu_toom4_top_handoff_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom4_top_handoff_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom4_top_handoff_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom4_top_handoff_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom4_top_handoff_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-toom4-top-handoff-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=toom4-top-handoff") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=toom4-top-inner-l64d2-vs-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-toom4-top-l64d2") != NULL);
        CHECK(strstr(report->results[index].detail, "candLeaf=64") != NULL);
        CHECK(strstr(report->results[index].detail, "candDepth=2") != NULL);
        CHECK(strstr(report->results[index].detail, "baseLeaf=48") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-toom4-top-reuse-l64d2") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-toom4-top-reuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom4-top-handoff") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom4-top-inner-handoff-map") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom4-top-fdiv-pt") == 0) {
        saw_mul_large_cpu_toom4_top_fdiv_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom4_top_fdiv_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom4_top_fdiv_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom4_top_fdiv_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom4_top_fdiv_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-toom4-top-fdiv-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=toom4-top-fdiv") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=toom4-top-factored-div-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-toom4-top-fdiv-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-toom4-top-reuse-factored-div-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-toom4-top-reuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom4-top-factored-div") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom4-top-factored-exact-div") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom4-top-vs-cmb-pt") == 0) {
        saw_mul_large_cpu_toom4_top_vs_cmb_point_probe = 1;
        if (report->results[index].digits == 24103) saw_mul_large_cpu_toom4_top_vs_cmb_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom4_top_vs_cmb_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom4_top_vs_cmb_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom4_top_vs_cmb_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-toom4-top-vs-cmb-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=toom4-top-vs-cmb") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=toom4-top-reuse-vs-combo-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=full-ws-toom4-top-reuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-toom4-top-reuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-combo-reuse-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom4-top-vs-combo-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom4-top-reuse-vs-combo-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 32768 ||
            report->results[index].digits == 65536) {
      ...
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-lower-point") == 0) {
        ...
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_cmb_lower_point5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_cmb_lower_point8192_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-lower-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-lower-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-lower-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=2") != NULL);
        CHECK(strstr(report->results[index].detail, "candDepth=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "hashGate=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=lower-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-depth2") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=full-ws-leaf64") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "candBaseRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-lower-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-lower-window") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 5639) {
          CHECK(strstr(report->results[index].detail, "sizeRole=deterministic-random-spot") != NULL);
        } else {
          CHECK(strstr(report->results[index].detail, "sizeRole=power2-anchor") != NULL);
        }
      }
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-route-point") == 0) {
        saw_mul_large_cpu_toom_cmb_route_point_probe = 1;
        if (report->results[index].digits == 4096) saw_mul_large_cpu_toom_cmb_route_point4096_probe = 1;
        else if (report->results[index].digits == 5639) saw_mul_large_cpu_toom_cmb_route_point5639_probe = 1;
        else if (report->results[index].digits == 8192) saw_mul_large_cpu_toom_cmb_route_point8192_probe = 1;
        else if (report->results[index].digits == 11717) saw_mul_large_cpu_toom_cmb_route_point11717_probe = 1;
        else if (report->results[index].digits == 16384) saw_mul_large_cpu_toom_cmb_route_point16384_probe = 1;
        else if (report->results[index].digits == 24103) saw_mul_large_cpu_toom_cmb_route_point24103_probe = 1;
        else if (report->results[index].digits == 32768) saw_mul_large_cpu_toom_cmb_route_point32768_probe = 1;
        else if (report->results[index].digits == 52163) saw_mul_large_cpu_toom_cmb_route_point52163_probe = 1;
        else if (report->results[index].digits == 65536) saw_mul_large_cpu_toom_cmb_route_point65536_probe = 1;
        else CHECK(0);
        CHECK(report->results[index].parity_verified);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].sample_count == 9);
        CHECK(strstr(report->results[index].detail, "op=mul-cmb-route-point") != NULL);
        CHECK(strstr(report->results[index].detail, "parent=cmb-route-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-route-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=large-multiply-cpu-toom-combo-route-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=toom33-combo-full-window") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
        if (report->results[index].digits == 4096 ||
            report->results[index].digits == 8192 ||
      ...
      if (strcmp(report->results[index].operation, "format-route-tournament") == 0) {
      ...
      } else if (strcmp(report->results[index].operation, "square-route-audit") == 0) {
        saw_square_route_audit = 1;
        CHECK(strstr(report->results[index].detail, "op=square-route-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=current-scratch-square") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=current-scratch-square") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "sameRunAudit=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "featureGate=square-route-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpClue=mpn_sqr-product-baseline") != NULL);
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        if (strstr(report->results[index].detail, "policy=current-default-1000-16384") != NULL) {
          saw_square_route_audit_large = 1;
          CHECK(report->results[index].sample_count == 4);
          CHECK(report->results[index].digits == 16384);
          CHECK(strstr(report->results[index].detail, "sizes=1000,4096,8192,16384") != NULL);
          CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
          CHECK(strstr(report->results[index].detail, "hashSafe=36/36") != NULL);
        } else if (strstr(report->results[index].detail, "policy=current-default-1000") != NULL) {
          saw_square_route_audit_focused = 1;
          CHECK(report->results[index].sample_count == 1);
          CHECK(report->results[index].digits == 1000);
          CHECK(strstr(report->results[index].detail, "sizes=1000") != NULL);
          CHECK(strstr(report->results[index].detail, "sizeCount=1") != NULL);
          CHECK(strstr(report->results[index].detail, "hashSafe=9/9") != NULL);
        } else {
          CHECK(0);
        }
      ...
      CHECK(report->results[index].gmp_us > 0);
      CHECK(report->results[index].speed_ratio > 0.0);
      if (strcmp(report->results[index].operation, "mul-large-toom-cmb-map-ctrl") == 0) {
        CHECK(report->results[index].max_allowed_speed_ratio == 1.10);
      } else {
        CHECK(report->results[index].max_allowed_speed_ratio == 1.0);
      }
      ...
      CHECK(strstr(report->results[index].detail, "thresholdSafety=forced-neighbor") != NULL ||
        ...
        strstr(report->results[index].detail, "thresholdSafety=smoke-window") != NULL ||
        strstr(report->results[index].detail, "thresholdSafety=transition-window") != NULL);
      CHECK(strstr(report->results[index].detail, "forcedCandidate=yes") != NULL);
      CHECK(strstr(report->results[index].detail, "ratioMethod=paired-median") != NULL);
      CHECK(strstr(report->results[index].detail, "adoption=") != NULL);
      if (strcmp(report->results[index].operation, "format-policy-safety") == 0 ||
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-depth-scout") == 0) {
        saw_mul_large_cpu_toom_depth_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-depth-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-depth3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-leaf-scout") == 0) {
        saw_mul_large_cpu_toom_leaf_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-leaf-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-leaf96-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-leaf48-scout") == 0) {
        saw_mul_large_cpu_toom_leaf48_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-leaf48-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-leaf48-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-div2-scout") == 0) {
        saw_mul_large_cpu_toom_div2_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-div2-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-div2-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-div3-scout") == 0) {
        saw_mul_large_cpu_toom_div3_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-div3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-div3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-div2-div3-scout") == 0) {
        saw_mul_large_cpu_toom_div2_div3_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-div2-div3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-div2-div3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-leaf48-scout") == 0) {
        saw_mul_large_cpu_toom_cmb_leaf48_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-leaf48-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-leaf48-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-depth3-scout") == 0) {
        saw_mul_large_cpu_toom_cmb_depth3_scout_probe = 1;
        CHECK(report->results[index].sample_count == 6);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-depth3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-depth3-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=6") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l48d3-full") == 0) {
        saw_mul_large_cpu_toom_cmb_l48d3_full_probe = 1;
        CHECK(report->results[index].sample_count == 9);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-l48d3-full") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l48d3-full-window-ge4096") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-hand") == 0) {
        saw_mul_large_cpu_toom_cmb_hand_probe = 1;
        CHECK(report->results[index].sample_count == 9);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-hand") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-handoff-l48d3-ge32768") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=4096,5639,8192,11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=9") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=4096") != NULL);
        ...
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-tourn") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-upper-tournament-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "requiredStablePairs=8/9") != NULL);
        CHECK(strstr(report->results[index].detail, "safeSizes=") != NULL);
        CHECK(strstr(report->results[index].detail, "hashSafe=288/288") != NULL);
        CHECK(strstr(report->results[index].detail, "hashGate=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "parity=matched") != NULL);
        CHECK(strstr(report->results[index].detail, "forcedCandidate=yes") != NULL);
        CHECK(strstr(report->results[index].detail, "thresholdSafety=upper-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=best-combo-upper-tournament") != NULL);
        ...
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=tournament-winner-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=48") != NULL);
        CHECK(strstr(report->results[index].detail, "depths=4,4,3,3") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        CHECK(strstr(report->results[index].detail, "samples=9") != NULL);
        CHECK(strstr(report->results[index].detail, "requiredStablePairs=8/9") != NULL);
        CHECK(strstr(report->results[index].detail, "safeSizes=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-reuse-map") == 0) {
        saw_mul_large_cpu_toom_cmb_reuse_map_probe = 1;
        CHECK(report->results[index].sample_count == 9);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-reuse-map") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-map-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=4096,5639,8192,11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=9") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=4096") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-troute") == 0) {
        saw_mul_large_cpu_toom_cmb_transition_route_probe = 1;
        CHECK(report->results[index].sample_count == 2);
        CHECK(report->results[index].digits == 16384);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-troute") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-map-transition-ge11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-tctrl") == 0) {
        saw_mul_large_cpu_toom_cmb_transition_control_probe = 1;
        CHECK(report->results[index].sample_count == 2);
        CHECK(report->results[index].digits == 16384);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].max_allowed_speed_ratio == 1.10);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-tctrl") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=2") != NULL);
        CHECK(strstr(report->results[index].detail, "focus=transition-route-noise") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "currentBaseline=current-scratch-mul") != NULL);
        CHECK(strstr(report->results[index].detail, "oracle=mpz_mul") != NULL);
        CHECK(strstr(report->results[index].detail, "controlRatioMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "controlWorstMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "candCurrentMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "currentGmpMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "maxWorstPairRatio=") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-gmpctrl") == 0) {
        saw_mul_large_cpu_toom_cmb_gmp_control_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-gmpctrl") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "gmpControlSafety=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "candGmpDuplicateMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "currentGmpMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpControlRatioMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "gmpControlWorstMax=") != NULL);
        CHECK(strstr(report->results[index].detail, "maxWorstPairRatio=") != NULL);
        CHECK(strstr(report->results[index].detail, "ratioMethod=paired-median") != NULL);
        CHECK(strstr(report->results[index].detail, "timingMode=rotating-batch") != NULL);
        CHECK(strstr(report->results[index].detail, "sameInput=yes") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-gmptrans") == 0) {
        saw_mul_large_cpu_toom_cmb_gmp_transition_probe = 1;
        CHECK(report->results[index].sample_count == 2);
        CHECK(report->results[index].digits == 16384);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-gmptrans") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=11717,16384") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=2") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=11717") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-neg2") == 0) {
        saw_mul_large_cpu_toom_cmb_neg2_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-neg2") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom5-top-reuse") == 0) {
        saw_mul_large_cpu_toom5_top_reuse_probe = 1;
        CHECK(report->results[index].sample_count == 2);
        CHECK(report->results[index].digits == 8192);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom5-top-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom5-top-reuse-smoke-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=5639,8192") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=2") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=5639") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=toom5-top-reuse-l32d2-smoke") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=32") != NULL);
        CHECK(strstr(report->results[index].detail, "depthLimit=2") != NULL);
        CHECK(strstr(report->results[index].detail, "operandFamilies=2") != NULL);
        ...
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-map") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-best-map-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=4096,5639,8192,11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=9") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=4096") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "thresholdSafety=full-window") != NULL);
        CHECK(strstr(report->results[index].detail, "candidate=full-ws-combo-best-map-l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "activeCandidate=best-known-by-size") != NULL);
        CHECK(strstr(report->results[index].detail, "lowerCandidate=full-ws-combo-l64d2") != NULL);
        CHECK(strstr(report->results[index].detail, "middleCandidate=full-ws-combo-l48d4") != NULL);
        CHECK(strstr(report->results[index].detail, "upperCandidate=full-ws-combo-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "baseline=current-scratch-mul") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-map-ctrl") == 0) {
        saw_mul_large_cpu_toom_cmb_map_control_probe = 1;
        CHECK(report->results[index].sample_count == 2);
        CHECK(report->results[index].digits == 32768);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(report->results[index].max_allowed_speed_ratio == 1.10);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-map-ctrl") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-best-map-control-24103-32768") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=2") != NULL);
        CHECK(strstr(report->results[index].detail, "focus=worst-pair-outliers") != NULL);
        CHECK(strstr(report->results[index].detail, "routePolicy=l64d2-l48d4-l48d3") != NULL);
        CHECK(strstr(report->results[index].detail, "cut24103=leaf48depth4") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l48d4-scout") == 0) {
        saw_mul_large_cpu_toom_cmb_l48d4_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-l48d4-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l48d4-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l32d4-scout") == 0) {
        saw_mul_large_cpu_toom_cmb_l32d4_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-l32d4-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l32d4-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-ipdiv") == 0) {
        saw_mul_large_cpu_toom_cmb_ipdiv_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-ipdiv") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-inplace-div-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-l64d3-scout") == 0) {
        saw_mul_large_cpu_toom_cmb_l64d3_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-l64d3-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-l64d3-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom4-top") == 0) {
        saw_mul_large_cpu_toom4_top_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom4-top") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom4-top-reuse") == 0) {
        saw_mul_large_cpu_toom4_top_reuse_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom4-top-reuse") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom4-top-handoff") == 0) {
        saw_mul_large_cpu_toom4_top_handoff_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom4-top-handoff") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom4-top-fdiv") == 0) {
        saw_mul_large_cpu_toom4_top_fdiv_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom4-top-fdiv") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom4-top-vs-cmb") == 0) {
        saw_mul_large_cpu_toom4_top_vs_cmb_scout_probe = 1;
        CHECK(report->results[index].sample_count == 4);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom4-top-vs-cmb") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=4") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=24103") != NULL);
        ...
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-lower-scout") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-lower-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=4096,5639,8192") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=3") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=4096") != NULL);
        CHECK(strstr(report->results[index].detail, "leafThreshold=64") != NULL);
        CHECK(strstr(report->results[index].detail, "baseDepth=2") != NULL);
        ...
        CHECK(strstr(report->results[index].detail, "noAutoRoute=1") != NULL);
        CHECK(strstr(report->results[index].detail, "replacementReady=false") != NULL);
      } else if (strcmp(report->results[index].operation, "mul-large-toom-cmb-route-audit") == 0) {
        saw_mul_large_cpu_toom_cmb_route_audit_probe = 1;
        CHECK(report->results[index].sample_count == 9);
        CHECK(report->results[index].digits == 65536);
        CHECK(!report->results[index].replacement_ready);
        CHECK(strcmp(report->results[index].adoption, "observe-only") == 0);
        CHECK(strstr(report->results[index].detail, "op=mul-large-toom-cmb-route-audit") != NULL);
        CHECK(strstr(report->results[index].detail, "policy=full-workspace-combo-route-ge4096") != NULL);
        CHECK(strstr(report->results[index].detail, "sizes=4096,5639,8192,11717,16384,24103,32768,52163,65536") != NULL);
        CHECK(strstr(report->results[index].detail, "sizeCount=9") != NULL);
        CHECK(strstr(report->results[index].detail, "minDigits=4096") != NULL);
...
#if defined(_MSC_VER) && defined(_M_X64)
  CHECK(saw_mul_large_cpu_toom_full_deep_point_probe);
  CHECK(saw_mul_large_cpu_toom_full_deep_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_full_deep_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_full_deep_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_full_deep_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_full_deep_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_full_deep_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_full_deep_audit_probe);
  CHECK(saw_mul_large_cpu_toom_depth_point_probe);
  CHECK(saw_mul_large_cpu_toom_depth_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_depth_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_depth_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_depth_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_depth_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_depth_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_depth_scout_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_point_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_leaf_scout_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_point_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_leaf48_scout_probe);
  CHECK(saw_mul_large_cpu_toom_div2_point_probe);
  CHECK(saw_mul_large_cpu_toom_div2_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_div2_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_div2_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_div2_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_div2_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_div2_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_div2_scout_probe);
  CHECK(saw_mul_large_cpu_toom_div3_point_probe);
  CHECK(saw_mul_large_cpu_toom_div3_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_div3_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_div3_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_div3_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_div3_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_div3_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_div3_scout_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_point_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_div2_div3_scout_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_leaf48_scout_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_depth3_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_depth3_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_depth3_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_depth3_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_depth3_point32768_probe);
  ...
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point4096_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point5639_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point8192_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d3_full_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point4096_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point5639_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point8192_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_hand_point32768_probe);
  ...
  CHECK(saw_mul_large_cpu_toom_cmb_tournament_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_tournament_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_tournament_l64d2_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_tournament_l48d3_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_tournament_l48d4_probe);
  ...
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point4096_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point5639_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point8192_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_reuse_map_point32768_probe);
  ...
  CHECK(saw_mul_large_cpu_toom_cmb_transition_route_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_transition_route_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_transition_route_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_transition_route_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_transition_control_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_transition_control_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_transition_control_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_transition_control_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_control_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_control_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_control_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_control_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_control_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_control_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_transition_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_transition_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_transition_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_gmp_transition_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_neg2_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_neg2_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_neg2_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_neg2_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_neg2_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_neg2_probe);
  CHECK(saw_mul_large_cpu_toom5_top_reuse_point_probe);
  ...
  CHECK(saw_mul_large_cpu_toom_cmb_map_point8192_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_control_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_control_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_control_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_map_control_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d4_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d4_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d4_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d4_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d4_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l48d4_scout_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l32d4_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l32d4_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l32d4_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l32d4_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l32d4_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l32d4_scout_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_ipdiv_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_ipdiv_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_ipdiv_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_ipdiv_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_ipdiv_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_ipdiv_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l64d3_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l64d3_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l64d3_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l64d3_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l64d3_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_l64d3_scout_probe);
  CHECK(saw_mul_large_cpu_toom4_top_point_probe);
  CHECK(saw_mul_large_cpu_toom4_top_point24103_probe);
  CHECK(saw_mul_large_cpu_toom4_top_point32768_probe);
  CHECK(saw_mul_large_cpu_toom4_top_point52163_probe);
  CHECK(saw_mul_large_cpu_toom4_top_point65536_probe);
  CHECK(saw_mul_large_cpu_toom4_top_scout_probe);
  CHECK(saw_mul_large_cpu_toom4_top_reuse_point_probe);
  CHECK(saw_mul_large_cpu_toom4_top_reuse_point24103_probe);
  CHECK(saw_mul_large_cpu_toom4_top_reuse_point32768_probe);
  CHECK(saw_mul_large_cpu_toom4_top_reuse_point52163_probe);
  CHECK(saw_mul_large_cpu_toom4_top_reuse_point65536_probe);
  CHECK(saw_mul_large_cpu_toom4_top_reuse_scout_probe);
  CHECK(saw_mul_large_cpu_toom4_top_handoff_point_probe);
  CHECK(saw_mul_large_cpu_toom4_top_handoff_point24103_probe);
  CHECK(saw_mul_large_cpu_toom4_top_handoff_point32768_probe);
  CHECK(saw_mul_large_cpu_toom4_top_handoff_point52163_probe);
  CHECK(saw_mul_large_cpu_toom4_top_handoff_point65536_probe);
  CHECK(saw_mul_large_cpu_toom4_top_handoff_scout_probe);
  CHECK(saw_mul_large_cpu_toom4_top_fdiv_point_probe);
  CHECK(saw_mul_large_cpu_toom4_top_fdiv_point24103_probe);
  CHECK(saw_mul_large_cpu_toom4_top_fdiv_point32768_probe);
  CHECK(saw_mul_large_cpu_toom4_top_fdiv_point52163_probe);
  CHECK(saw_mul_large_cpu_toom4_top_fdiv_point65536_probe);
  CHECK(saw_mul_large_cpu_toom4_top_fdiv_scout_probe);
  CHECK(saw_mul_large_cpu_toom4_top_vs_cmb_point_probe);
  CHECK(saw_mul_large_cpu_toom4_top_vs_cmb_point24103_probe);
  CHECK(saw_mul_large_cpu_toom4_top_vs_cmb_point32768_probe);
  CHECK(saw_mul_large_cpu_toom4_top_vs_cmb_point52163_probe);
  CHECK(saw_mul_large_cpu_toom4_top_vs_cmb_point65536_probe);
  CHECK(saw_mul_large_cpu_toom4_top_vs_cmb_scout_probe);
  ...
  CHECK(saw_mul_large_cpu_toom_cmb_lower_point8192_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_lower_scout_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point4096_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point5639_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point8192_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point11717_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point16384_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point24103_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point32768_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point52163_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_point65536_probe);
  CHECK(saw_mul_large_cpu_toom_cmb_route_audit_probe);
#endif
  ...
  CHECK(saw_mul_dense_leaf_vs_scan_probe);
  ...
  CHECK(saw_mul_large_cpu_campaign_probe);
  CHECK(saw_mul_large_cpu_campaign4096_probe);
  CHECK(saw_mul_large_cpu_campaign5639_probe);
  CHECK(saw_mul_large_cpu_campaign8192_probe);
  CHECK(saw_mul_large_cpu_campaign11717_probe);
  CHECK(saw_mul_large_cpu_campaign16384_probe);
  CHECK(saw_mul_large_cpu_campaign24103_probe);
  CHECK(saw_mul_large_cpu_campaign32768_probe);
  CHECK(saw_mul_large_cpu_campaign52163_probe);
  CHECK(saw_mul_large_cpu_campaign65536_probe);
#if defined(_MSC_VER) && defined(_M_X64)
  CHECK(saw_mul_large_cpu_toom_branch_probe);
  CHECK(saw_mul_large_cpu_toom_branch4096_probe);
  CHECK(saw_mul_large_cpu_toom_branch5639_probe);
  CHECK(saw_mul_large_cpu_toom_branch8192_probe);
  CHECK(saw_mul_large_cpu_toom_branch11717_probe);
  CHECK(saw_mul_large_cpu_toom_branch16384_probe);
  CHECK(saw_mul_large_cpu_toom_branch24103_probe);
  CHECK(saw_mul_large_cpu_toom_branch32768_probe);
  CHECK(saw_mul_large_cpu_toom_branch52163_probe);
  CHECK(saw_mul_large_cpu_toom_branch65536_probe);
  CHECK(saw_mul_large_cpu_toom_view_branch_probe);
  CHECK(saw_mul_large_cpu_toom_view_branch4096_probe);
  CHECK(saw_mul_large_cpu_toom_view_branch5639_probe);
  CHECK(saw_mul_large_cpu_toom_view_branch8192_probe);
  CHECK(saw_mul_large_cpu_toom_view_branch11717_probe);
  CHECK(saw_mul_large_cpu_toom_view_branch16384_probe);
  CHECK(saw_mul_large_cpu_toom_view_branch24103_probe);
  ...
  CHECK(saw_mul_large_cpu_toom_full_workspace_probe);
  CHECK(saw_mul_large_cpu_toom_full_workspace4096_probe);
  CHECK(saw_mul_large_cpu_toom_full_workspace5639_probe);
  CHECK(saw_mul_large_cpu_toom_full_workspace8192_probe);
  CHECK(saw_mul_large_cpu_toom_full_workspace11717_probe);
  CHECK(saw_mul_large_cpu_toom_full_workspace16384_probe);
  CHECK(saw_mul_large_cpu_toom_full_workspace24103_probe);
...
#if defined(_MSC_VER) && defined(_M_X64)
  CHECK(strstr(json, "mul-large-toom-full-deep-point") != NULL);
  ...
  CHECK(strstr(json, "mul-large-toom-depth-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-depth-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-depth-scout") != NULL);
  CHECK(strstr(json, "full-workspace-depth3-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-leaf-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-leaf-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-leaf-scout") != NULL);
  CHECK(strstr(json, "full-workspace-leaf96-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-leaf48-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-leaf48-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-leaf48-scout") != NULL);
  CHECK(strstr(json, "full-workspace-leaf48-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-div2-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-div2-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-div2-scout") != NULL);
  CHECK(strstr(json, "full-workspace-div2-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-div3-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-div3-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-div3-scout") != NULL);
  CHECK(strstr(json, "full-workspace-div3-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-div2-div3-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-div2-div3-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-div2-div3-scout") != NULL);
  CHECK(strstr(json, "full-workspace-div2-div3-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-leaf48-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-leaf48-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-leaf48-scout") != NULL);
  CHECK(strstr(json, "full-workspace-combo-leaf48-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-depth3-point") != NULL);
  ...
  CHECK(strstr(json, "mul-large-toom-cmb-l48d3-fpt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-l48d3-full") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-l48d3-full") != NULL);
  CHECK(strstr(json, "full-workspace-combo-l48d3-full-window-ge4096") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-hand-pt") != NULL);
  ...
  CHECK(strstr(json, "mul-large-toom-cmb-troute-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-troute") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-reuse-map-transition") != NULL);
  CHECK(strstr(json, "full-workspace-combo-reuse-map-transition-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-tctrl-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-tctrl") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-reuse-map-transition-control") != NULL);
  CHECK(strstr(json, "full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-gmpctrl-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-gmpctrl") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-gmp-control") != NULL);
  CHECK(strstr(json, "full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-gmptrans-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-gmptrans") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-gmp-control-transition") != NULL);
  CHECK(strstr(json, "full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-neg2-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-neg2") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-neg2") != NULL);
  CHECK(strstr(json, "full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom5-top-reuse-pt") != NULL);
  ...
  CHECK(strstr(json, "mul-large-toom-cmb-map") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-best-map") != NULL);
  CHECK(strstr(json, "full-workspace-combo-best-map-ge4096") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-map-ctrl-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-map-ctrl") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-best-map-control") != NULL);
  CHECK(strstr(json, "full-workspace-combo-best-map-control-24103-32768") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-l48d4-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-l48d4-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-l48d4-scout") != NULL);
  CHECK(strstr(json, "full-workspace-combo-l48d4-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-l32d4-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-l32d4-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-l32d4-scout") != NULL);
  CHECK(strstr(json, "full-workspace-combo-l32d4-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-ipdiv-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-ipdiv") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-inplace-interp") != NULL);
  CHECK(strstr(json, "full-workspace-combo-inplace-div-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-l64d3-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-l64d3-scout") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-l64d3-scout") != NULL);
  CHECK(strstr(json, "full-workspace-combo-l64d3-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom4-top-scout") != NULL);
  CHECK(strstr(json, "full-workspace-toom4-top-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-reuse-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-reuse") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom4-top-reuse") != NULL);
  CHECK(strstr(json, "full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-handoff-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-handoff") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom4-top-handoff") != NULL);
  CHECK(strstr(json, "full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-fdiv-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-fdiv") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom4-top-factored-div") != NULL);
  CHECK(strstr(json, "full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-vs-cmb-pt") != NULL);
  CHECK(strstr(json, "mul-large-toom4-top-vs-cmb") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom4-top-vs-combo-reuse") != NULL);
  CHECK(strstr(json, "full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
  ...
  CHECK(strstr(json, "full-workspace-combo-lower-ge4096") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-route-point") != NULL);
  CHECK(strstr(json, "mul-large-toom-cmb-route-audit") != NULL);
  CHECK(strstr(json, "large-multiply-cpu-toom-combo-route-audit") != NULL);
  CHECK(strstr(json, "full-workspace-combo-route-ge4096") != NULL);
...
#if defined(_MSC_VER) && defined(_M_X64)
  CHECK(strstr(tsv, "mul-large-toom-full-deep-point") != NULL);
  ...
  CHECK(strstr(tsv, "mul-large-toom-depth-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-depth-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-depth-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-depth3-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-leaf-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-leaf-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-leaf-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-leaf96-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-leaf48-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-leaf48-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-leaf48-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-leaf48-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-div2-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-div2-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-div2-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-div2-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-div3-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-div3-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-div3-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-div3-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-div2-div3-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-div2-div3-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-div2-div3-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-div2-div3-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-leaf48-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-leaf48-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-leaf48-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-leaf48-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-depth3-point") != NULL);
  ...
  CHECK(strstr(tsv, "mul-large-toom-cmb-l48d3-fpt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-l48d3-full") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-l48d3-full") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-l48d3-full-window-ge4096") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-hand-pt") != NULL);
  ...
  CHECK(strstr(tsv, "mul-large-toom-cmb-troute-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-troute") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-reuse-map-transition") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-reuse-map-transition-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-tctrl-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-tctrl") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-reuse-map-transition-control") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-gmpctrl-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-gmpctrl") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-gmp-control") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-gmptrans-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-gmptrans") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-gmp-control-transition") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-neg2-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-neg2") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-neg2") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
  ...
  CHECK(strstr(tsv, "mul-large-toom-cmb-map") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-best-map") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-best-map-ge4096") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-map-ctrl-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-map-ctrl") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-best-map-control") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-best-map-control-24103-32768") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-l48d4-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-l48d4-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-l48d4-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-l48d4-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-l32d4-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-l32d4-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-l32d4-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-l32d4-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-ipdiv-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-ipdiv") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-inplace-interp") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-inplace-div-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-l64d3-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-l64d3-scout") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-l64d3-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-l64d3-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom4-top-scout") != NULL);
  CHECK(strstr(tsv, "full-workspace-toom4-top-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-reuse-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-reuse") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom4-top-reuse") != NULL);
  CHECK(strstr(tsv, "full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-handoff-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-handoff") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom4-top-handoff") != NULL);
  CHECK(strstr(tsv, "full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-fdiv-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-fdiv") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom4-top-factored-div") != NULL);
  CHECK(strstr(tsv, "full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-vs-cmb-pt") != NULL);
  CHECK(strstr(tsv, "mul-large-toom4-top-vs-cmb") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom4-top-vs-combo-reuse") != NULL);
  CHECK(strstr(tsv, "full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
  ...
  CHECK(strstr(tsv, "full-workspace-combo-lower-ge4096") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-route-point") != NULL);
  CHECK(strstr(tsv, "mul-large-toom-cmb-route-audit") != NULL);
  CHECK(strstr(tsv, "large-multiply-cpu-toom-combo-route-audit") != NULL);
  CHECK(strstr(tsv, "full-workspace-combo-route-ge4096") != NULL);
  ...
  CHECK(strstr(benchmark_json, "\"toom3-full-workspace\"") != NULL);
  CHECK(strstr(benchmark_json, "\"toom3-full-workspace-div2\"") != NULL);
  CHECK(strstr(benchmark_json, "\"toom3-full-workspace-div3\"") != NULL);
  CHECK(strstr(benchmark_json, "\"toom3-full-workspace-div2-div3\"") != NULL);
  ...
  CHECK(strstr(benchmark_json, "mul-large-toom-full-deep-point") != NULL);
  ...
  CHECK(strstr(benchmark_json, "mul-large-toom-depth-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-depth3-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-leaf-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-leaf96-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-leaf48-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-leaf48-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-div2-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-div2-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-div3-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-div2-div3-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-div2-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-leaf48-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-leaf48-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-depth3-point") != NULL);
  ...
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l48d3-fpt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l48d3-full") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-l48d3-full") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-l48d3-full-window-ge4096") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-hand-pt") != NULL);
  ...
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-troute-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-troute") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-reuse-map-transition") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-reuse-map-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-tctrl-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-tctrl") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-reuse-map-transition-control") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-gmpctrl-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-gmpctrl") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-gmp-control") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-gmptrans-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-gmptrans") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-gmp-control-transition") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-neg2-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-neg2") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-neg2") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom5-top-reuse-pt") != NULL);
  ...
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-map") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-best-map") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-best-map-ge4096") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-map-ctrl-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-map-ctrl") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-best-map-control") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-best-map-control-24103-32768") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l48d4-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l48d4-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-l48d4-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-l48d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l32d4-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l32d4-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-l32d4-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-l32d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-ipdiv-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-ipdiv") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-inplace-interp") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-inplace-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l64d3-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-l64d3-scout") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-l64d3-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-l64d3-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom4-top-scout") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-toom4-top-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-reuse-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-handoff-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-fdiv-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-fdiv") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom4-top-factored-div") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-vs-cmb-pt") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom4-top-vs-cmb") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom4-top-vs-combo-reuse") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
  ...
  CHECK(strstr(benchmark_json, "full-workspace-combo-lower-ge4096") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-route-point") != NULL);
  CHECK(strstr(benchmark_json, "mul-large-toom-cmb-route-audit") != NULL);
  CHECK(strstr(benchmark_json, "large-multiply-cpu-toom-combo-route-audit") != NULL);
  CHECK(strstr(benchmark_json, "full-workspace-combo-route-ge4096") != NULL);
#endif
  ...
  CHECK(strstr(benchmark_tsv, "mul-large-cpu-campaign") != NULL);
  CHECK(strstr(benchmark_tsv, "karatsuba-split-view-workspace") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-campaign") != NULL);
#if defined(_MSC_VER) && defined(_M_X64)
  CHECK(strstr(benchmark_tsv, "mul-large-cpu-toom-branch") != NULL);
  CHECK(strstr(benchmark_tsv, "recursive-toom3+unroll4") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-branch") != NULL);
  CHECK(strstr(benchmark_tsv, "toomWorkspaceRatio=") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-cpu-toom-view-branch") != NULL);
  ...
  CHECK(strstr(benchmark_tsv, "mul-large-cpu-toom-full-ws") != NULL);
  CHECK(strstr(benchmark_tsv, "recursive-toom3+unroll4-full-workspace") != NULL);
  ...
  CHECK(strstr(benchmark_tsv, "mul-large-toom-full-deep-point") != NULL);
  ...
  CHECK(strstr(benchmark_tsv, "mul-large-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-depth-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-depth3-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-leaf-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-leaf96-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-leaf48-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-leaf48-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-div2-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-div2-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-div3-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-div2-div3-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-div2-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-leaf48-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-leaf48-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-depth3-scout") != NULL);
  ...
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l48d3-full") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l48d3-fpt") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-l48d3-full") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-l48d3-full-window-ge4096") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-hand") != NULL);
  ...
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-troute-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-troute") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-reuse-map-transition") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-reuse-map-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-tctrl-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-tctrl") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-reuse-map-transition-control") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-gmpctrl-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-gmpctrl") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-gmp-control") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-gmptrans-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-gmptrans") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-gmp-control-transition") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-neg2-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-neg2") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-neg2") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom5-top-reuse-pt") != NULL);
  ...
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-map") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-best-map") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-best-map-ge4096") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-map-ctrl-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-map-ctrl") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-best-map-control") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-best-map-control-24103-32768") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l48d4-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l48d4-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-l48d4-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-l48d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l32d4-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l32d4-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-l32d4-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-l32d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-ipdiv") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-ipdiv-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-inplace-interp") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-inplace-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l64d3-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-l64d3-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-l64d3-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-l64d3-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom4-top-scout") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-toom4-top-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-reuse-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-handoff-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-fdiv") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-fdiv-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom4-top-factored-div") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-vs-cmb") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom4-top-vs-cmb-pt") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom4-top-vs-combo-reuse") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
  ...
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-lower-ge4096") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-route-audit") != NULL);
  CHECK(strstr(benchmark_tsv, "mul-large-toom-cmb-route-point") != NULL);
  CHECK(strstr(benchmark_tsv, "large-multiply-cpu-toom-combo-route-audit") != NULL);
  CHECK(strstr(benchmark_tsv, "full-workspace-combo-route-ge4096") != NULL);
  CHECK(strstr(benchmark_tsv, "thresholdSafety=active-window") != NULL);
  CHECK(strstr(benchmark_tsv, "hashSafe=108/108") != NULL);
  CHECK(strstr(benchmark_tsv, "hashSafe=18/18") != NULL);
#endif
  CHECK(strstr(benchmark_tsv, "sizeRole=deterministic-random-spot") != NULL);
  CHECK(strstr(benchmark_tsv, "digits=5639") != NULL);
  CHECK(strstr(benchmark_tsv, "digits=11717") != NULL);
  CHECK(strstr(benchmark_tsv, "digits=24103") != NULL);
  CHECK(strstr(benchmark_tsv, "digits=52163") != NULL);
  CHECK(strstr(benchmark_tsv, "noAutoRoute=1") != NULL);
#if defined(_MSC_VER) && defined(_M_X64)
  ...
  CHECK(strstr(benchmark_frontier, "mul-large-toom-full-deep-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-full-deep-audit") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-depth-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-leaf-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-leaf48-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-div2-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-div3-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-div2-div3-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-leaf48-point") != NULL);
  ...
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-l48d3-fpt") != NULL);
  ...
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-troute-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-troute") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-tctrl-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-tctrl") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-gmpctrl-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-gmpctrl") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-gmptrans-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-gmptrans") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-neg2-pt") != NULL);
  ...
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-map-ctrl") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-l48d4-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-l48d4-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-l32d4-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-l32d4-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-ipdiv-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-ipdiv") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-l64d3-point") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-l64d3-scout") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-reuse-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-handoff-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-fdiv-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-fdiv") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-vs-cmb-pt") != NULL);
  CHECK(strstr(benchmark_frontier, "mul-large-toom4-top-vs-cmb") != NULL);
  ...
  CHECK(strstr(benchmark_frontier, "mul-large-toom-cmb-route-audit") != NULL);
#endif
...
#if defined(_MSC_VER) && defined(_M_X64)
  CHECK(strstr(benchmark_progress, "mul-large-cpu-toom-branch") != NULL);
  ...
  CHECK(strstr(benchmark_progress, "mul-large-toom-full-deep-point") != NULL);
  ...
  CHECK(strstr(benchmark_progress, "mul-large-toom-depth-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-depth3-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-leaf-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-leaf96-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-leaf48-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-leaf48-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-div2-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-div2-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-div3-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-div2-div3-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-div2-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-leaf48-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-leaf48-ge11717") != NULL);
  ...
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-l48d3-fpt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-l48d3-full") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-l48d3-full-window-ge4096") != NULL);
  ...
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-troute-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-troute") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-reuse-map-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-tctrl-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-tctrl") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-gmpctrl-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-gmpctrl") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-gmptrans-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-gmptrans") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-neg2-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-neg2") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
  ...
  CHECK(strstr(benchmark_progress, "full-workspace-combo-best-map-ge4096") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-map-ctrl-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-map-ctrl") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-best-map-control-24103-32768") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-l48d4-point") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-l48d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-l32d4-point") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-l32d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-ipdiv-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-ipdiv") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-inplace-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-l64d3-point") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-l64d3-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-toom4-top-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-reuse-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-handoff-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-fdiv-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-fdiv") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-vs-cmb-pt") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom4-top-vs-cmb") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
  ...
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-route-point") != NULL);
  CHECK(strstr(benchmark_progress, "mul-large-toom-cmb-route-audit") != NULL);
  CHECK(strstr(benchmark_progress, "full-workspace-combo-route-ge4096") != NULL);
#endif
  ...
  CHECK(strstr(benchmark_progress_tsv, "mul-large-cpu-campaign") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "karatsuba-split-view-workspace") != NULL);
#if defined(_MSC_VER) && defined(_M_X64)
  CHECK(strstr(benchmark_progress_tsv, "mul-large-cpu-toom-branch") != NULL);
  ...
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-full-deep-point") != NULL);
  ...
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-depth-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-depth-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-depth3-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-leaf-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-leaf-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-leaf96-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-leaf48-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-leaf48-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-div2-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-div2-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-div2-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-div3-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-div3-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-div2-div3-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-div2-div3-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-div2-div3-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-leaf48-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-leaf48-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-leaf48-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-depth3-point") != NULL);
  ...
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l48d3-fpt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l48d3-full") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-l48d3-full") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-l48d3-full-window-ge4096") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-hand-pt") != NULL);
  ...
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-troute-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-troute") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-reuse-map-transition") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-reuse-map-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-tctrl-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-tctrl") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-reuse-map-transition-control") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-reuse-self-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-gmpctrl-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-gmpctrl") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-gmp-control") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-reuse-gmp-control-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-gmptrans-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-gmptrans") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-gmp-control-transition") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-reuse-gmp-control-transition-ge11717") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-neg2-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-neg2") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-neg2") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-reuse-neg2-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom5-top-reuse-pt") != NULL);
  ...
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-map") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-best-map") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-best-map-ge4096") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-map-ctrl-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-map-ctrl") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-best-map-control") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-best-map-control-24103-32768") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l48d4-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l48d4-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-l48d4-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-l48d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l32d4-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l32d4-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-l32d4-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-l32d4-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-ipdiv-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-ipdiv") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-inplace-interp") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-inplace-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l64d3-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-l64d3-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-l64d3-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-l64d3-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom4-top-scout") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-toom4-top-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-reuse-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom4-top-reuse") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-toom4-top-reuse-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-handoff-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom4-top-handoff") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-toom4-top-handoff-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-fdiv-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-fdiv") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom4-top-factored-div") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-toom4-top-factored-div-upper-ge24103") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-vs-cmb-pt") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom4-top-vs-cmb") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom4-top-vs-combo-reuse") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-toom4-top-vs-combo-reuse-upper-ge24103") != NULL);
  ...
  CHECK(strstr(benchmark_progress_tsv, "full-workspace-combo-lower-ge4096") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-route-point") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "mul-large-toom-cmb-route-audit") != NULL);
  CHECK(strstr(benchmark_progress_tsv, "large-multiply-cpu-toom-combo-route-audit") != NULL);
...
int main(int argc, char **argv) {
  ...
  RUN_NATIVE_TEST(test_scratch_bigint_toom5_top_reuse_probe_oracle, "bigint");
