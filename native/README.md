...
The 64-bit scratch migration is intentionally gated per operation. Add/sub/mul can become replacement-ready where the benchmark proves it, and `mod-u32`/`gcd-u32`/`divmod-u32`/`powmod-u32` use reciprocal multiply-high division with exact correction to avoid slow hardware division in the hot path. Rows that do not beat GMP remain `oracle-only`; evidence is not permission to route production solver accounting away from GMP.
