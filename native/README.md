...
Measured keep/drop decisions are recorded in [`PERFORMANCE.md`](PERFORMANCE.md) so rejected scouts do not get repeated as if they were still unexplored. A paper-style draft of the bounded GMP/MPIR multiply wins lives in [`GMP_MULTIPLY_PAPER_DRAFT.md`](GMP_MULTIPLY_PAPER_DRAFT.md); it is intentionally framed as verified pocket evidence, not a global replacement claim.
