 ...
 * exact division-by-two interpolation steps with checked bit shifts.
 *
 * This diagnostic probe keeps production multiply unchanged and exists only to
 * isolate lower-level Toom interpolation cost. depth_limit bounds recursion for
 * benchmark safety. out may alias either input. Returns 1 on success and 0 on
 * allocation failure, unsupported operand shape, or failed exact-divisibility
 * checks.
 */
XRAY_API int xray_bigint_mul_toom3_unroll4_recursive_full_workspace_div2_probe(XrayScratchBigInt *out, const XrayScratchBigInt *left, const XrayScratchBigInt *right, size_t leaf_threshold, size_t depth_limit);

/**
 * Multiply through the full-workspace recursive Toom-3 probe while replacing
 * the exact division-by-three interpolation step with an odd-limb exact
 * division shortcut.
 *
 * This diagnostic probe keeps production multiply unchanged and exists only to
 * isolate lower-level Toom interpolation cost. depth_limit bounds recursion for
...
/**
 * Multiply through the full-workspace recursive Toom-3 probe while replacing
 * the checked division-by-two and division-by-three interpolation shortcuts
 * with in-place exact updates.
 *
 * This diagnostic probe keeps production multiply unchanged and exists only to
 * isolate interpolation temporary-copy cost. depth_limit bounds recursion for
 * benchmark safety. out may alias either input. Returns 1 on success and 0 on
 * allocation failure, unsupported operand shape, or failed exact-divisibility
 * checks.
 */
XRAY_API int xray_bigint_mul_toom3_unroll4_recursive_full_workspace_inplace_div2_div3_probe(XrayScratchBigInt *out, const XrayScratchBigInt *left, const XrayScratchBigInt *right, size_t leaf_threshold, size_t depth_limit);
...
/**
 * Multiply through the reusable full-workspace recursive Toom-3 combo probe
 * while evaluating the outer Toom-3 non-unit point at -2 instead of +2.
 *
 * This diagnostic probe is benchmark-only: production multiply remains
 * unchanged, out may alias either input, and the route keeps the checked exact
 * `/2` plus `/3` interpolation shortcuts. It exists to measure whether the
 * signed evaluation/interpolation arithmetic shape changes the upper
 * large-multiply window. Returns 1 on success and 0 on allocation failure,
 * unsupported operand shape, invalid workspace, or failed exact-divisibility
 * checks.
 */
XRAY_API int xray_bigint_mul_toom3_unroll4_recursive_full_workspace_reuse_neg2_div2_div3_probe(XrayScratchBigInt *out, const XrayScratchBigInt *left, const XrayScratchBigInt *right, size_t leaf_threshold, size_t depth_limit, XrayBigIntMulWorkspace *workspace);
 ...
 *
 * This diagnostic probe keeps production multiply unchanged and exists only to
 * measure whether a broader top split helps the upper large-multiply window.
 * depth_limit bounds the Toom-3 recursion used inside point products. out may
 * alias either input. Returns 1 on success and 0 on allocation failure,
 * unsupported operand shape, or failed exact-divisibility checks.
 */
XRAY_API int xray_bigint_mul_toom4_top_full_workspace_probe(XrayScratchBigInt *out, const XrayScratchBigInt *left, const XrayScratchBigInt *right, size_t leaf_threshold, size_t depth_limit);
 ...
 *
 * This diagnostic probe keeps production multiply unchanged and exists only to
 * isolate recursive workspace allocation/setup cost from the top-level Toom-4
 * arithmetic shape. depth_limit bounds the Toom-3 recursion used inside point
 * products. out may alias either input. Returns 1 on success and 0 on
 * allocation failure, unsupported operand shape, invalid workspace, or failed
 * exact-divisibility checks.
 */
 ...
 * gmpClue, controlSafety, thresholdSafety, hashGate, safeSizeChunks,
 * longestSafeSizeChunkSpan, safeSizeChunkTotalSpan, and blockerReason so
 * external tools can group optimizer evidence by size and route shape without
 * scraping the detail field. This is intended for external tools that need the
 * same benchmark-progress classification as xray_benchmark_progress_tsv_text()
 * without parsing the human-readable digest. The caller owns the returned string
 * and must release it with xray_free().
 */
